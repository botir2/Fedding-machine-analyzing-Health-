...
#################################main function###################################################
    ...
    def main(self):
        ...
        last_day = datetime.now() - timedelta(days=1)
        las = last_day.strftime('%Y-%m-%d')
        details = collection.find({'date': {"$regex": self.thre_days_ago}})
        details2 = collection.find({'date': {"$regex": self.two_days_ago}})
        # details = collection.find()
        # details = collection.aggregate([{"$match": {'date': {"$regex": "2018-12-27"}}}])
        rest = 0
        stand = 0
        runing = 0
        walking = 0
